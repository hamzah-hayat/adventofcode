...
func TestLargestArea(t *testing.T) {
	...
	for i := range inputs {
		t.Run("Test using Inputs "+strconv.Itoa(i), func(t *testing.T) {
			testOutput := FindLargestArea(inputs[i])
...
func TestNearestNeighbours(t *testing.T) {
	inputs1 := []Point{Point{18, 20}, Point{0, 0}, Point{4, 6}}
	inputs2 := [][]Point{[]Point{Point{12, 18}, Point{1, 2}}, []Point{Point{1, 1}}, []Point{Point{1, 1}, Point{1, 6}, Point{8, 3}, Point{3, 4}, Point{5, 5}, Point{8, 9}}}
	outputs := []Point{Point{12, 18}, Point{1, 1}, Point{5, 5}}
